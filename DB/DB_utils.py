...
# V
def search_inventory_rate(cursor, s_id):
    query = """
...
def generate_employee_id(cursor):
    query = "SELECT 'EM' || lpad(nextval('employee_id_seq')::text, 10, '0');"
    cursor.execute(query)
    return cursor.fetchone()[0]

def db_register_employee(cursor, e_name, start_date, password, mgr_id, role):
    e_id = generate_employee_id(cursor)
    cmd = """
...
def add_rate(cursor, score, year, s_id, e_id):
    ...
    VALUES (%s, %s, %s, %s)
    """
    cursor.execute(query, (score, year, s_id, e_id))  # Use parameterized query to avoid SQL injection
    cursor.connection.commit()  # Commit the transaction
